...
export function ShieldModel({ isRotating = true }: ShieldModelProps) {
  ...
  useEffect(() => {
    ...
    // Lock shackle
    const lockShackleGeometry = new THREE.TorusGeometry(0.15, 0.03, 16, 32, Math.PI)
    ...
    const animate = () => {
      ...
      if (glow) {
        ...
        glow.scale.set(1 + Math.sin(time * 0.5) * 0.05, 1 + Math.sin(time * 0.5) * 0.05, 1)
      }
  ...
  function createShieldShape(scale = 1) {
    ...
    shape.bezierCurveTo(1 * scale, 1.5 * scale, 1.5 * scale, 1 * scale, 1.5 * scale, 0 * scale)
    shape.bezierCurveTo(1.5 * scale, -1 * scale, 0.75 * scale, -1.5 * scale, 0 * scale, -1.75 * scale)
    shape.bezierCurveTo(-0.75 * scale, -1.5 * scale, -1.5 * scale, -1 * scale, -1.5 * scale, 0 * scale)
    shape.bezierCurveTo(-1.5 * scale, 1 * scale, -1 * scale, 1.5 * scale, 0 * scale, 1.5 * scale)
